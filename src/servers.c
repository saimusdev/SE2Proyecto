...
void create_servers (void) {
    ...
    s11_mutex = PTHREAD_MUTEX_INITIALIZER;
	...
	s12_mutex = PTHREAD_MUTEX_INITIALIZER;
    ...
    s21_mutex = PTHREAD_MUTEX_INITIALIZER;
	...
	s22_mutex = PTHREAD_MUTEX_INITIALIZER;
}
...
void server1_func_2 (unsigned char task_id) {
	...
	printf("task #%d: S12\n", task_id);
	delay_ms(s12_comp_time); /* doing stuff */
}
